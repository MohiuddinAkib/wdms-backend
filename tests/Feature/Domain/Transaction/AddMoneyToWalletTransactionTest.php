...
class AddMoneyToWalletTransactionTest extends TestCase
{
    ...
    public function test_should_be_authenticated_to_add_money_to_wallet(): void
    {
        ...
        $response = $this->postJson(route('transactions.deposit', $wallet->getKey()), [
    ...
    public function test_should_provide_valid_wallet_denomination_to_add_money_to_wallet(): void
    {
        ...
        $response
            ->assertUnprocessable()
            ->assertJsonValidationErrors([
                'denominations.0.denomination_id' => 'The selected denominations.0.denomination_id is invalid.',
            ]);
    }

    public function test_should_prevent_duplicate_denomination_to_add_money_to_wallet(): void
    {
        $user = User::factory()->create();
        Sanctum::actingAs($user);
        $wallet = WalletFactory::new()->withUserUuid($user->uuid)
